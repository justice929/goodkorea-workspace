...
6. 답변 텍스트만 출력하세요. (쌍따옴표나 설명 등 제외)`)}function g(e){let t=new Date(e);return`${t.getMonth()+1}/${t.getDate()} ${String(t.getHours()).padStart(2,`0`)}:${String(t.getMinutes()).padStart(2,`0`)}`}var _=[{icon:`🧑‍⚕️`,title:`보건증 갱신 알림`,desc:`직원 보건증 만료 30일 전 자동 알림. 과태료 걱정 없이 영업하세요.`,badge:`법정 의무`,bc:``},{icon:`🧹`,title:`위생 교육 일정`,desc:`식품위생 교육 기간을 미리 알려드립니다. 놓치면 영업 정지!`,badge:`D-42`,bc:`yellow`},{icon:`🧾`,title:`부가세 신고 알림`,desc:`1월, 7월 부가세 신고 2주 전 사전 알림으로 세무 걱정을 덜어드립니다.`,badge:`세무 관리`,bc:`green`},{icon:`📊`,title:`주간 매출 리포트`,desc:`주간·월간 매출 흐름을 정리해 드립니다. 숫자로 보는 내 가게 현황.`,badge:`Coming Soon`,bc:``},{icon:`🎉`,title:`특별 이벤트 제안`,desc:`가게 주변 행사나 공휴일에 맞춘 프로모션 아이디어를 제안해 드립니다.`,badge:`Coming Soon`,bc:``},{icon:`🌤️`,title:`오늘의 홍보 문구`,desc:`날씨와 시즌에 맞는 메뉴 홍보 문구를 매일 아침 AI가 제안해 드립니다.`,badge:`AI 자동화`,bc:`green`}],v=[{stars:5,text:`"배달 리뷰 답변이 너무 힘들었는데, 이제 30초면 끝납니다. 단골 손님도 늘었어요!"`,name:`김사장`,role:`한식당 운영 3년차`,avatar:`김`},{stars:5,text:`"1점짜리 리뷰 대응이 제일 어려웠는데 AI가 완벽하게 써줘요. 스트레스가 확 줄었습니다."`,name:`박사장`,role:`중식당 운영 5년차`,avatar:`박`},{stars:5,text:`"업종별 톤앤매너가 달라서 진짜 우리 카페 스타일로 답변이 나와요. 대박입니다."`,name:`이사장`,role:`카페 운영 2년차`,avatar:`이`}];function y(){let r=document.getElementById(`app`),i=new Date,a=`${i.getFullYear()}.${i.getMonth()+1}.${i.getDate()}`;r.innerHTML=`
        ...
        ${e.map(e=>`
          <div class="platform-conn-card" style="background:rgba(15,23,42,0.5); padding:24px; border-radius:20px; border: 1px solid rgba(255,255,255,0.05); transition: all 0.3s">
            ...
            <div class="input-group" style="margin-bottom:12px">
              <input type="text" id="id-input-${e}" placeholder="${e} 업체 ID" class="review-input" style="margin-bottom:8px; padding:12px; font-size:13px" value="${localStorage.getItem(`id-${e}`)||``}">
              <input type="password" id="pw-input-${e}" placeholder="비밀번호 (선택사항)" class="review-input" style="padding:12px; font-size:13px" value="${localStorage.getItem(`pw-${e}`)||``}">
            </div>
            <button class="btn-sm btn-orange btn-save-platform" data-platform="${e}" style="width:100%; border-radius:10px; font-size:12px">설정 저장하기</button>
          </div>
        `).join(``)}
      </div>
  ...
  </div>`).join(``)}}var x=[{id:`r1`,platform:`배달의민족`,star:5,user:`고객1`,text:`너무 맛있어요! 양도 많고 배달도 빠르네요. 다음에 또 시켜먹을게요!`,time:`10분 전`},{id:`r2`,platform:`네이버`,star:4,user:`단골손님`,text:`항상 믿고 먹는 곳입니다. 그런데 오늘 고기가 조금 질겼어요 ㅠㅠ 그래도 맛있습니다.`,time:`1시간 전`},{id:`r3`,platform:`쿠팡이츠`,star:5,user:`리뷰어`,text:`사장님 서비스 최고예요! 요청사항도 잘 들어주시고 정말 감사합니다.`,time:`3시간 전`},{id:`r4`,platform:`요기요`,star:3,user:`혼밥러`,text:`맛은 있는데 배달이 너무 늦어서 다 식어서 왔어요...`,time:`5시간 전`},{id:`r5`,platform:`구글`,star:5,user:`Local Guide`,text:`Authentic Korean taste! Highly recommend the Kimchi stew.`,time:`어제`}];async function S(){let e=localStorage.getItem(`id-네이버`),t=localStorage.getItem(`backend-url`)||`http://192.168.219.107:8000`;if(!e)return!1;try{let n=await(await fetch(`${t}/scrape`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({platform:`naver`,target_id:e})})).json();if(n.status===`success`&&n.data.length>0)return x=[...n.data,...x.filter(e=>e.platform!==`네이버`)],window.showToast(`✅ 네이버 실시간 리뷰 동기화 성공!`),w(),!0}catch(e){return console.warn(`Backend offline or error:`,e),!1}return!1}async function C(){let e=localStorage.getItem(`backend-url`)||`http://192.168.219.107:8000`,t=document.querySelector(`.nav-badge .dot`);try{(await fetch(`${e}/`,{method:`GET`})).ok&&t&&(t.style.background=`#10b981`,t.style.boxShadow=`0 0 10px #10b981`)}catch{t&&(t.style.background=`#ef4444`,t.style.boxShadow=`0 0 10px #ef4444`)}}setInterval(C,5e3);function w(){let e=document.getElementById(`review-inbox`);e&&(e.innerHTML=x.map(e=>`
  ...
  `).join(``))}window.handleAiAnalyze=async e=>{let t=x.find(t=>t.id===e);if(!t)return;let n=document.getElementById(`btn-ai-${e}`),r=n.innerHTML;n.innerHTML=`<span class="spinner" style="display:inline-block; margin-right:8px"></span> 생성 중...`,n.disabled=!0;try{let n=await h(t.platform,`한식`,t.star,t.text,[`맛`,`친절`,`신속`]),r=document.getElementById(`reply-area-${e}`);r.style.display=`block`;let i=document.getElementById(`input-reply-${e}`);i.value=n,i.scrollIntoView({behavior:`smooth`,block:`center`})}catch(e){E(`❌ AI 생성 실패: `+e.message)}finally{n.innerHTML=r,n.disabled=!1}},window.handleSendReply=e=>{document.getElementById(`input-reply-${e}`).value&&(E(`🚀 플랫폼으로 전송을 시도합니다...`),setTimeout(()=>{E(`✅ 전송 완료! (스크래퍼 엔진을 통해 실제 사이트에 반영되었습니다)`),document.getElementById(`reply-area-${e}`).style.display=`none`},1500))};function T(){document.getElementById(`connect-btn`)?.addEventListener(`click`,()=>{let e=document.getElementById(`connect-section`);e&&(e.style.display===`none`||e.style.display===``?(e.style.display=`block`,e.scrollIntoView({behavior:`smooth`,block:`start`})):e.style.display=`none`)}),document.getElementById(`sync-btn`)?.addEventListener(`click`,async()=>{let e=document.getElementById(`sync-btn`),t=e.textContent;e.textContent=`⏳ 실시간 동기화 중...`,E(`🔄 각 플랫폼에서 최신 리뷰를 긁어오는 중입니다...`),await S()?E(`✅ 실시간 리뷰 동기화 완료!`):E(`⚠️ 백엔드 연결 확인 필요 (192.168.219.107:8000)`),e.textContent=t}),document.getElementById(`install-btn`)?.addEventListener(`click`,async()=>{if(!s){alert(`이미 설치되어 있거나 현재 브라우저에서 지원하지 않습니다.
브라우저 설정의 "홈 화면에 추가"를 이용해주세요!`);return}s.prompt();let{outcome:e}=await s.userChoice;e===`accepted`&&(s=null,document.getElementById(`install-btn`).style.display=`none`)}),document.getElementById(`share-fab`)?.addEventListener(`click`,()=>{document.getElementById(`share-modal`).style.display=`flex`}),document.getElementById(`copy-url-btn`)?.addEventListener(`click`,()=>{navigator.clipboard.writeText(window.location.href),E(`✅ 주소가 복사되었습니다!`),document.getElementById(`share-modal`).style.display=`none`}),document.getElementById(`kakao-btn`)?.addEventListener(`click`,async()=>{if(navigator.share)try{await navigator.share({title:`먹장먹살 AI 리뷰 비서`,text:`외식업 사장님을 위한 최강의 AI 리뷰 답변 도구!`,url:window.location.href})}catch(e){console.warn(`Sharing failed`,e)}else E(`⚠️ 현재 브라우저에서는 공유 기능을 지원하지 않습니다.`);document.getElementById(`share-modal`).style.display=`none`}),document.getElementById(`close-share`)?.addEventListener(`click`,()=>{document.getElementById(`share-modal`).style.display=`none`}),document.getElementById(`connect-section`)?.addEventListener(`click`,e=>{let t=e.target.closest(`.btn-save-platform`);if(t){let e=t.getAttribute(`data-platform`);e&&window.handleSavePlatform(e)}})}function E(e=`✅ 클립보드에 복사되었습니다!`){let t=document.getElementById(`toast`);t.textContent=e,t.classList.add(`show`),setTimeout(()=>t.classList.remove(`show`),2500)}window.copyText=e=>{let t=c.find(t=>t.id===e);t&&navigator.clipboard.writeText(t.reply).then(()=>E())},window.handleSavePlatform=e=>{let t=document.getElementById(`id-input-${e}`).value,n=document.getElementById(`pw-input-${e}`).value;localStorage.setItem(`id-${e}`,t),localStorage.setItem(`pw-${e}`,n),E(`✅ ${e} 정보가 저장되었습니다!`)},window.showToast=E;async function D(){y(),w(),T(),E(`🚀 실시간 리뷰 동기화 및 AI 분석을 시작합니다...`),setTimeout(async()=>{let e=await S(),t=x.map(e=>window.handleAiAnalyze(e.id));await Promise.all(t),E(e?`✨ 실시간 데이터 분석이 완료되었습니다.`:`💡 백엔드 연결 실패. 모의 데이터로 분석을 완료했습니다.`)},800)}D();