...
`;window.setFilter=e=>{o=e,y()},window.openPlatformReviews=async e=>{i=e,r=`reviews`,y(),window.syncReviews(e)},window.syncReviews=async e=>{d(`${e} 리뷰를 가져오고 있습니다...`);try{let t=localStorage.getItem(`backend-url`)||`http://localhost:8000`,r=localStorage.getItem(`id-${e}`)||`1876527582`,i=await(await fetch(`${t}/scrape`,{method:`POST`,headers:{"Content-Type":`application/json`,"X-API-Key":n},body:JSON.stringify({platform:e===`네이버`?`naver`:e,target_id:r})})).json();i.status===`success`&&(a=i.data.map(t=>({...t,platform:e})),y(),d(`리뷰 동기화 완료!`))}catch{d(`백엔드 연결 실패. 모의 데이터를 표시합니다.`,!0),a=[{id:`1`,platform:e,user:`고마워요`,star:5,text:`진짜 맛있네요! 또 올게요.`,time:`방금`,replied:!1},{id:`2`,platform:e,user:`단골님`,star:4,text:`항상 잘 먹고 있습니다.`,time:`어제`,replied:!0}],y()}},window.handleFinalSubmit=async e=>{let t=a.find(t=>t.id===e);if(!t)return;let r=document.getElementById(`text-${e}`)?.value?.trim();if(!r)return d(`답변 내용을 입력해주세요.`,!0);try{d(`답글을 등록하고 있습니다...`);let e=localStorage.getItem(`backend-url`)||`http://localhost:8000`;(await(await fetch(`${e}/reply`,{method:`POST`,headers:{"Content-Type":`application/json`,"X-API-Key":n},body:JSON.stringify({platform:t.platform===`네이버`?`naver`:t.platform,place_id:localStorage.getItem(`id-${t.platform}`)||``,review_id:t.id,reply_text:r})})).json()).status===`success`?(t.replied=!0,d(`답글이 등록되었습니다!`),y()):d(`답글 등록에 실패했습니다.`,!0)}catch{d(`백엔드 연결 실패.`,!0)}},window.saveConnectId=e=>{let t=document.getElementById(`connect-id-${e}`);t&&(localStorage.setItem(`id-${e}`,t.value),d(`${e} ID가 저장되었습니다.`))},window.saveBackendUrl=()=>{let e=document.getElementById(`connect-backend-url`);e&&(localStorage.setItem(`backend-url`,e.value),d(`백엔드 서버 주소가 저장되었습니다.`))},window.handleCreateDraft=async t=>{let n=a.find(e=>e.id===t);if(!n)return;d(`AI 답변을 생성하고 있습니다...`);let r=``;try{r=(await(await fetch(e,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({contents:[{parts:[{text:`당신은 친절한 음식점 사장님입니다. 아래 고객 리뷰에 대해 감사하고 진심 어린 답글을 3~5문장으로 작성해주세요.\n\n리뷰: "${n.text}"`}]}]})})).json()).candidates?.[0]?.content?.parts?.[0]?.text||`답변 생성에 실패했습니다.`}catch{r=`네트워크 오류로 AI 답변 생성에 실패했습니다. 직접 입력해주세요.`}document.getElementById(`reply-result-${t}`).style.display=`block`,document.getElementById(`text-${t}`).value=r,d(`AI 답변이 생성되었습니다!`)};function y(){let e=document.getElementById(`app`);e.innerHTML=`
    <div class="bg-orb bg-orb-1"></div>
    <div class="bg-orb bg-orb-2"></div>
    ${p()}
    <div id="view-container">
      ${r===`login`?m():r===`signup`?h():r===`main`?g():r===`connect`?v():_()}
    </div>
