...
`},_=()=>{let e=a.filter(e=>e.platform===i&&(o===`all`||!e.replied));return`
        ...
        <div class="tool-card" style="margin-bottom:32px; padding:32px; border-left: 6px solid ${u[e.platform]}; border-radius:32px">
          <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px">
            <div style="display:flex; align-items:center; gap:12px;">
              ...
              <span style="font-size:12px; color:var(--text-3);">${e.time}</span>
            </div>
            <span style="color:var(--yellow); font-size:14px;">${`⭐`.repeat(e.star)}</span>
          </div>
          ...
          ${e.images&&e.images.length>0?`
            <div style="display:flex; gap:10px; margin-bottom:20px; overflow-x:auto; padding-bottom:8px;">
              ${e.images.map(e=>`
                <img src="${e}" style="width:120px; height:120px; object-fit:cover; border-radius:12px; border:1px solid rgba(255,255,255,0.1)">
              `).join(``)}
            </div>
          `:``}
          ...
          `:`
            <button class="generate-btn" style="background:var(--grad-1)" onclick="handleCreateDraft('${e.id}')">✨ AI 맞춤 답글 생성하기</button>
          `}
          <div id="reply-result-${e.id}" style="display:none; margin-top:24px; animation: slideUp 0.4s ease-out;">
            <div style="font-size:12px; color:var(--text-3); margin-bottom:8px; font-weight:700;">🤖 AI 추천 답변 (자유롭게 수정 가능)</div>
            <textarea id="text-${e.id}" class="review-input" rows="6" style="margin-bottom:16px; background:rgba(255,255,255,0.03); border-color:var(--grad-1)"></textarea>
            <div style="display:flex; gap:12px;">
              <button class="btn-sm btn-outline" style="flex:1; padding:16px;" onclick="document.getElementById('reply-result-${e.id}').style.display='none'">취소</button>
              <button class="btn-sm btn-orange" style="flex:2; padding:16px; background:var(--grad-1); border:none;" onclick="handleFinalSubmit('${e.id}')">플랫폼에 즉시 등록</button>
            </div>
          </div>
        </div>
      `).join(``)}
  ...
  <section class="tool-section" style="max-width: 700px;">
    ...
    ${c.map(e=>{if(v.includes(e)){let t=(localStorage.getItem(`id-${e}`)||``).split(`|`);return`
        ...
        </div>`}let t=e===`네이버`?(localStorage.getItem(`sp-네이버`)||``).split(`|`):[];return`
      <div class="tool-card" style="margin-bottom:20px; padding:28px;">
        ...
        <div style="margin-top:20px; padding-top:20px; border-top:1px solid var(--border);">
          ...
          <input type="text" class="review-input" id="sp-naver-id"
            placeholder="네이버 아이디" value="${t[0]||``}" style="margin-bottom:8px;">
          <label style="font-size:12px; color:var(--text-3); display:block; margin-bottom:4px;">비밀번호</label>
...
`;window.setFilter=e=>{o=e,b()},window.openPlatformReviews=async e=>{i=e,r=`reviews`,b(),window.syncReviews(e)},window.syncReviews=async e=>{d(`${e} 리뷰 수집 중...`),console.log(`[DEBUG] SYNC START: ${e}`);try{let t=localStorage.getItem(`backend-url`)||`http://localhost:8000`,n=localStorage.getItem(`id-${e}`)||``;(n===`1876527582`||n===`rnw8080`)&&(console.log(`[DEBUG] Old bad ID detected. Clearing...`),localStorage.removeItem(`id-${e}`),n=``);let r=n||(e===`네이버`?`1078667259`:``);if(!r){d(`${e} 연동 아이디를 설정해주세요.`,!0);return}console.log(`[DEBUG] Fetching: ${t}/scrape | ID: ${r}`);let i=await fetch(`${t}/scrape`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({platform:e===`네이버`?`naver`:e,target_id:r})});if(!i.ok)throw Error(`HTTP Error: ${i.status}`);let o=await i.json();console.log(`[DEBUG] Received Data:`,o),o.status===`success`?(a=o.data.map(t=>({...t,platform:e})),b(),d(`${e} 리뷰 ${a.length}개 수집 완료!`)):d(`수집 실패: ${o.detail}`,!0)}catch(e){console.error(`[DEBUG] Sync Error:`,e),d(`서버 연결 실패! 백엔드가 켜져있는지 확인해주세요.`,!0),a=[],b()}},window.handleFinalSubmit=async e=>{let t=a.find(t=>t.id===e);if(!t)return;let r=document.getElementById(`text-${e}`)?.value?.trim();if(!r)return d(`답변 내용을 입력해주세요.`,!0);try{d(`답글을 등록하고 있습니다...`);let e=localStorage.getItem(`backend-url`)||`http://localhost:8000`;(await(await fetch(`${e}/reply`,{method:`POST`,headers:{"Content-Type":`application/json`,"X-API-Key":n},body:JSON.stringify({platform:t.platform===`네이버`?`naver`:t.platform,place_id:localStorage.getItem(`id-${t.platform}`)||``,review_id:t.id,reply_text:r,credentials:t.platform===`네이버`&&localStorage.getItem(`sp-네이버`)||``})})).json()).status===`success`?(t.replied=!0,d(`답글이 등록되었습니다!`),b()):d(`답글 등록에 실패했습니다.`,!0)}catch{d(`백엔드 연결 실패.`,!0)}},window.saveConnectId=e=>{let t=document.getElementById(`connect-id-${e}`);if(t){if(v.includes(e)){let n=document.getElementById(`connect-pw-${e}`)?.value||``;localStorage.setItem(`id-${e}`,`${t.value}|${n}`)}else localStorage.setItem(`id-${e}`,t.value);d(`${e} 정보가 저장되었습니다.`)}},window.saveNaverSmartPlace=()=>{let e=document.getElementById(`sp-naver-id`),t=document.getElementById(`sp-naver-pw`);e&&t&&(localStorage.setItem(`sp-네이버`,`${e.value}|${t.value}`),d(`스마트플레이스 계정이 저장되었습니다.`))},window.saveBackendUrl=()=>{let e=document.getElementById(`connect-backend-url`);e&&(localStorage.setItem(`backend-url`,e.value),d(`백엔드 서버 주소가 저장되었습니다.`))},window.handleCreateDraft=async t=>{let n=a.find(e=>e.id===t);if(!n)return;d(`AI 답변을 생성하고 있습니다...`);let r=``;try{let t=await fetch(e,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({contents:[{parts:[{text:`당신은 친절한 음식점 사장님입니다. 아래 고객 리뷰에 대해 감사하고 진심 어린 답글을 3~5문장으로 작성해주세요.\n\n리뷰: "${n.text}"`}]}]})}),i=await t.json();if(t.ok)r=i.candidates?.[0]?.content?.parts?.[0]?.text||`답변 생성에 실패했습니다.`;else{let e=i.error?.message||`HTTP ${t.status}`;d(`Gemini 오류: ${e}`,!0),r=`[오류] ${e}\n\n직접 답변을 입력해주세요.`}}catch(e){d(`네트워크 오류: ${e?.message||`알 수 없는 오류`}`,!0),r=`네트워크 오류로 생성에 실패했습니다. 직접 입력해주세요.`}document.getElementById(`reply-result-${t}`).style.display=`block`,document.getElementById(`text-${t}`).value=r,d(`AI 답변이 생성되었습니다!`)};function b(){let e=document.getElementById(`app`);e.innerHTML=`
