...
브라우저 설정의 "홈 화면에 추가"를 이용해주세요!`);return}c.prompt();let{outcome:e}=await c.userChoice;e===`accepted`&&(c=null,document.getElementById(`install-btn`).style.display=`none`)}),document.getElementById(`share-fab`)?.addEventListener(`click`,()=>{document.getElementById(`share-modal`).style.display=`flex`}),document.getElementById(`copy-url-btn`)?.addEventListener(`click`,()=>{navigator.clipboard.writeText(window.location.href),E(`✅ 주소가 복사되었습니다!`),document.getElementById(`share-modal`).style.display=`none`}),document.getElementById(`close-share`)?.addEventListener(`click`,()=>{document.getElementById(`share-modal`).style.display=`none`})}function E(e=`✅ 클립보드에 복사되었습니다!`){let t=document.getElementById(`toast`);t.textContent=e,t.classList.add(`show`),setTimeout(()=>t.classList.remove(`show`),2500)}window.copyText=e=>{let t=l.find(t=>t.id===e);t&&navigator.clipboard.writeText(t.reply).then(()=>E())},window.showToast=E;async function D(){b(),w(),T(),E(`🚀 실시간 리뷰 동기화 및 AI 분석을 시작합니다...`),setTimeout(async()=>{let e=await C(),t=S.map(e=>window.handleAiAnalyze(e.id));await Promise.all(t),E(e?`✨ 실시간 데이터 분석이 완료되었습니다.`:`💡 백엔드 연결 실패. 모의 데이터로 분석을 완료했습니다.`)},800)}D();