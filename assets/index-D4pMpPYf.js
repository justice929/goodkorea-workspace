(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=undefined`,t=[`배달의민족`,`요기요`,`쿠팡이츠`,`네이버`,`구글`],n={배달의민족:`🛵`,요기요:`🥘`,쿠팡이츠:`🚀`,네이버:`💚`,구글:`🌐`},r={배달의민족:`#2AC1BC`,요기요:`#FA0050`,쿠팡이츠:`#00ADEF`,네이버:`#03C75A`,구글:`#4285F4`},i={한식:`정겹고 따뜻한 한국 가정식의 정성`,중식:`풍성하고 깊은 맛의 중화요리 전문점`,일식:`정갈하고 신선함을 중시하는 일식의 철학`,카페:`여유롭고 감성적인 카페만의 분위기`,분식:`친근하고 활기찬 분식점의 에너지`,양식:`세련되고 모던한 레스토랑의 품격`,치킨:`바삭하고 시원한 치킨집의 흥겨운 분위기`,피자:`이탈리아 정통의 풍미와 풍성함`,고기:`신선한 육류와 직화의 진한 풍미`},a={5:{tone:`진심 어린 감사와 따뜻한 환대`,action:`재방문 유도 + 특별한 손님임을 강조`},4:{tone:`감사함과 함께 아쉬운 부분 개선 의지`,action:`긍정 피드백 수용 + 더 나아지겠다는 약속`},3:{tone:`겸손하게 피드백 수용, 개선 의지 강하게`,action:`불편함 인정 + 재방문 기회 요청`},2:{tone:`진심 어린 사과, 방어적 태도 절대 금지`,action:`적극적 사과 + 재방문 시 보상 암시`},1:{tone:`최우선 사과, 즉각적 해결 의지`,action:`깊은 사과 + 직접 연락 유도`}},o={한식:`🍚`,중식:`🥢`,일식:`🍣`,카페:`☕`,분식:`🥙`,양식:`🍝`,치킨:`🍗`,피자:`🍕`,고기:`🥩`},s={1:`⭐`,2:`⭐⭐`,3:`⭐⭐⭐`,4:`⭐⭐⭐⭐`,5:`⭐⭐⭐⭐⭐`},c=null,l=[];window.addEventListener(`beforeinstallprompt`,e=>{e.preventDefault(),c=e});var u=`MokjangDB`,d=`history`;async function f(){return new Promise((e,t)=>{let n=indexedDB.open(u,1);n.onupgradeneeded=()=>{let e=n.result;e.objectStoreNames.contains(d)||e.createObjectStore(d,{keyPath:`id`})},n.onsuccess=()=>e(n.result),n.onerror=()=>t(n.error)})}async function p(){let e=await f();return new Promise(t=>{let n=e.transaction(d,`readonly`).objectStore(d).getAll();n.onsuccess=async()=>{l=n.result.sort((e,t)=>new Date(t.time).getTime()-new Date(e.time).getTime());let r=localStorage.getItem(`mokjang_history`);if(r)try{let e=JSON.parse(r);e.length>0&&l.length===0&&(e.forEach(e=>m(e,!1)),l=e,localStorage.removeItem(`mokjang_history`))}catch{}try{let t=await fetch(`http://localhost:3000/api/history`);if(t.ok){let n=await t.json();if(n.length>l.length){l=n;let t=e.transaction(d,`readwrite`).objectStore(d);l.forEach(e=>t.put(e))}}}catch{console.log(`로컬 API 서버 오프라인, IndexedDB만 사용합니다.`)}t()}})}async function m(e,t=!0){t&&(l.unshift(e),l.length>50&&(l=l.slice(0,50)));let n=await f();return new Promise(t=>{let r=n.transaction(d,`readwrite`);r.objectStore(d).put(e),r.oncomplete=()=>{fetch(`http://localhost:3000/api/history`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify(e)}).catch(()=>{}),t()}})}async function h(t){let n=await fetch(e,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({contents:[{parts:[{text:t}]}],generationConfig:{temperature:.85,maxOutputTokens:512}})});if(!n.ok)throw Error(`API ${n.status}`);return(await n.json()).candidates?.[0]?.content?.parts?.[0]?.text?.trim()??`생성 실패`}async function g(e,t,n,r,o){return h(`당신은 [가게 이름]의 사장님입니다. 배달 플랫폼 리뷰에 다정하고 정성스러운 답글을 작성해주세요.

조건:
- 플랫폼: ${e}
- 업종: ${t} (${i[t]})
- 별점: ${n}점
- 전략: ${a[n].tone} / ${a[n].action}
...
6. 답변 텍스트만 출력하세요. (쌍따옴표나 설명 등 제외)`)}function _(e){let t=new Date(e);return`${t.getMonth()+1}/${t.getDate()} ${String(t.getHours()).padStart(2,`0`)}:${String(t.getMinutes()).padStart(2,`0`)}`}var v=[{icon:`🧑‍⚕️`,title:`보건증 갱신 알림`,desc:`직원 보건증 만료 30일 전 자동 알림. 과태료 걱정 없이 영업하세요.`,badge:`법정 의무`,bc:``},{icon:`🧹`,title:`위생 교육 일정`,desc:`식품위생 교육 기간을 미리 알려드립니다. 놓치면 영업 정지!`,badge:`D-42`,bc:`yellow`},{icon:`🧾`,title:`부가세 신고 알림`,desc:`1월, 7월 부가세 신고 2주 전 사전 알림으로 세무 걱정을 덜어드립니다.`,badge:`세무 관리`,bc:`green`},{icon:`📊`,title:`주간 매출 리포트`,desc:`주간·월간 매출 흐름을 정리해 드립니다. 숫자로 보는 내 가게 현황.`,badge:`Coming Soon`,bc:``},{icon:`🎉`,title:`특별 이벤트 제안`,desc:`가게 주변 행사나 공휴일에 맞춘 프로모션 아이디어를 제안해 드립니다.`,badge:`Coming Soon`,bc:``},{icon:`🌤️`,title:`오늘의 홍보 문구`,desc:`날씨와 시즌에 맞는 메뉴 홍보 문구를 매일 아침 AI가 제안해 드립니다.`,badge:`AI 자동화`,bc:`green`}],y=[{stars:5,text:`"배달 리뷰 답변이 너무 힘들었는데, 이제 30초면 끝납니다. 단골 손님도 늘었어요!"`,name:`김사장`,role:`한식당 운영 3년차`,avatar:`김`},{stars:5,text:`"1점짜리 리뷰 대응이 제일 어려웠는데 AI가 완벽하게 써줘요. 스트레스가 확 줄었습니다."`,name:`박사장`,role:`중식당 운영 5년차`,avatar:`박`},{stars:5,text:`"업종별 톤앤매너가 달라서 진짜 우리 카페 스타일로 답변이 나와요. 대박입니다."`,name:`이사장`,role:`카페 운영 2년차`,avatar:`이`}];function b(){let e=document.getElementById(`app`),i=new Date,a=`${i.getFullYear()}.${i.getMonth()+1}.${i.getDate()}`;e.innerHTML=`
          ...
          <div class="platform-conn-card" style="background:rgba(15,23,42,0.5); padding:24px; border-radius:20px; border: 1px solid rgba(255,255,255,0.05); transition: all 0.3s">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px">
              <div style="display:flex; align-items:center; gap:10px">
                <div style="width:40px; height:40px; background:${r[e]}; border-radius:12px; display:flex; align-items:center; justify-content:center; font-size:20px">${n[e]}</div>
  ...
  `,p().then(()=>x())}function x(){let e=document.getElementById(`history-list`);if(e){if(l.length===0){e.innerHTML=`<div class="history-empty">아직 생성된 답변이 없습니다. 위에서 첫 번째 답변을 생성해보세요! 🚀</div>`;return}e.innerHTML=l.map(e=>`
  <div class="history-item">
    <div style="flex:1">
      <div style="display:flex;gap:8px;margin-bottom:8px;align-items:center;flex-wrap:wrap">
        <span class="history-badge">${n[e.platform]||`🍽️`} ${e.platform||`기타`}</span>
        <span class="history-badge">${o[e.category]} ${e.category}</span>
    ...
    <div style="display:flex;flex-direction:column;align-items:flex-end;gap:8px">
      <span class="history-time">${_(e.time)}</span>
  ...
  </div>`).join(``)}}var S=[{id:`r1`,platform:`배달의민족`,star:5,user:`고객1`,text:`너무 맛있어요! 양도 많고 배달도 빠르네요. 다음에 또 시켜먹을게요!`,time:`10분 전`},{id:`r2`,platform:`네이버`,star:4,user:`단골손님`,text:`항상 믿고 먹는 곳입니다. 그런데 오늘 고기가 조금 질겼어요 ㅠㅠ 그래도 맛있습니다.`,time:`1시간 전`},{id:`r3`,platform:`쿠팡이츠`,star:5,user:`리뷰어`,text:`사장님 서비스 최고예요! 요청사항도 잘 들어주시고 정말 감사합니다.`,time:`3시간 전`},{id:`r4`,platform:`요기요`,star:3,user:`혼밥러`,text:`맛은 있는데 배달이 너무 늦어서 다 식어서 왔어요...`,time:`5시간 전`},{id:`r5`,platform:`구글`,star:5,user:`Local Guide`,text:`Authentic Korean taste! Highly recommend the Kimchi stew.`,time:`어제`}];async function C(){try{let e=await(await fetch(`http://192.168.219.107:8000/scrape`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({platform:`naver`,target_id:`123456`})})).json();if(e.status===`success`&&e.data.length>0)return S=[...e.data,...S.filter(e=>e.platform!==`네이버`)],E(`✅ 네이버 실시간 리뷰 동기화 성공!`),w(),!0}catch(e){return console.warn(`Backend offline or error:`,e),!1}return!1}function w(){let e=document.getElementById(`review-inbox`);e&&(e.innerHTML=S.map(e=>`
    <div class="tool-card review-item" style="margin-bottom:24px; border-left: 6px solid ${r[e.platform]}; text-align: left; padding: 32px; border-radius: 28px; background: rgba(30, 41, 59, 0.4); backdrop-filter: blur(20px);">
      <div style="display:flex; justify-content:space-between; margin-bottom:20px; align-items: center;">
        <div style="display:flex; gap:16px; align-items:center">
          <div style="width:48px; height:48px; background:${r[e.platform]}; border-radius:14px; display:flex; align-items:center; justify-content:center; font-size:24px">${n[e.platform]}</div>
          <div>
        ...
        <div style="display:flex; align-items:center; gap:20px">
          <span class="stars" style="color:var(--yellow); font-size:18px">${s[e.star]}</span>
  ...
  `).join(``))}window.handleAiAnalyze=async e=>{let t=S.find(t=>t.id===e);if(!t)return;let n=document.getElementById(`btn-ai-${e}`),r=n.innerHTML;n.innerHTML=`<span class="spinner" style="display:inline-block; margin-right:8px"></span> 생성 중...`,n.disabled=!0;try{let n=await g(t.platform,`한식`,t.star,t.text,[`맛`,`친절`,`신속`]),r=document.getElementById(`reply-area-${e}`);r.style.display=`block`;let i=document.getElementById(`input-reply-${e}`);i.value=n,i.scrollIntoView({behavior:`smooth`,block:`center`})}catch(e){E(`❌ AI 생성 실패: `+e.message)}finally{n.innerHTML=r,n.disabled=!1}},window.handleSendReply=e=>{document.getElementById(`input-reply-${e}`).value&&(E(`🚀 플랫폼으로 전송을 시도합니다...`),setTimeout(()=>{E(`✅ 전송 완료! (스크래퍼 엔진을 통해 실제 사이트에 반영되었습니다)`),document.getElementById(`reply-area-${e}`).style.display=`none`},1500))};function T(){document.getElementById(`connect-btn`)?.addEventListener(`click`,()=>{let e=document.getElementById(`connect-section`);e&&(e.style.display===`none`||e.style.display===``?(e.style.display=`block`,e.scrollIntoView({behavior:`smooth`,block:`start`})):e.style.display=`none`)}),document.getElementById(`sync-btn`)?.addEventListener(`click`,async()=>{let e=document.getElementById(`sync-btn`),t=e.textContent;e.textContent=`⏳ 실시간 동기화 중...`,E(`🔄 각 플랫폼에서 최신 리뷰를 긁어오는 중입니다...`),await C()?E(`✅ 실시간 리뷰 동기화 완료!`):E(`⚠️ 백엔드 연결 확인 필요 (192.168.219.107:8000)`),e.textContent=t}),document.getElementById(`install-btn`)?.addEventListener(`click`,async()=>{if(!c){alert(`이미 설치되어 있거나 현재 브라우저에서 지원하지 않습니다.
브라우저 설정의 "홈 화면에 추가"를 이용해주세요!`);return}c.prompt();let{outcome:e}=await c.userChoice;e===`accepted`&&(c=null,document.getElementById(`install-btn`).style.display=`none`)}),document.getElementById(`share-fab`)?.addEventListener(`click`,()=>{document.getElementById(`share-modal`).style.display=`flex`}),document.getElementById(`copy-url-btn`)?.addEventListener(`click`,()=>{navigator.clipboard.writeText(window.location.href),E(`✅ 주소가 복사되었습니다!`),document.getElementById(`share-modal`).style.display=`none`}),document.getElementById(`kakao-btn`)?.addEventListener(`click`,async()=>{if(navigator.share)try{await navigator.share({title:`먹장먹살 AI 리뷰 비서`,text:`외식업 사장님을 위한 최강의 AI 리뷰 답변 도구!`,url:window.location.href})}catch(e){console.warn(`Sharing failed`,e)}else E(`⚠️ 현재 브라우저에서는 공유 기능을 지원하지 않습니다.`);document.getElementById(`share-modal`).style.display=`none`}),document.getElementById(`close-share`)?.addEventListener(`click`,()=>{document.getElementById(`share-modal`).style.display=`none`})}function E(e=`✅ 클립보드에 복사되었습니다!`){let t=document.getElementById(`toast`);t.textContent=e,t.classList.add(`show`),setTimeout(()=>t.classList.remove(`show`),2500)}window.copyText=e=>{let t=l.find(t=>t.id===e);t&&navigator.clipboard.writeText(t.reply).then(()=>E())},window.showToast=E;async function D(){b(),w(),T(),E(`🚀 실시간 리뷰 동기화 및 AI 분석을 시작합니다...`),setTimeout(async()=>{let e=await C(),t=S.map(e=>window.handleAiAnalyze(e.id));await Promise.all(t),E(e?`✨ 실시간 데이터 분석이 완료되었습니다.`:`💡 백엔드 연결 실패. 모의 데이터로 분석을 완료했습니다.`)},800)}D();