...
6. 답변 텍스트만 출력하세요. (쌍따옴표나 설명 등 제외)`)}function _(e){let t=new Date(e);return`${t.getMonth()+1}/${t.getDate()} ${String(t.getHours()).padStart(2,`0`)}:${String(t.getMinutes()).padStart(2,`0`)}`}var v=[{icon:`🧑‍⚕️`,title:`보건증 갱신 알림`,desc:`직원 보건증 만료 30일 전 자동 알림. 과태료 걱정 없이 영업하세요.`,badge:`법정 의무`,bc:``},{icon:`🧹`,title:`위생 교육 일정`,desc:`식품위생 교육 기간을 미리 알려드립니다. 놓치면 영업 정지!`,badge:`D-42`,bc:`yellow`},{icon:`🧾`,title:`부가세 신고 알림`,desc:`1월, 7월 부가세 신고 2주 전 사전 알림으로 세무 걱정을 덜어드립니다.`,badge:`세무 관리`,bc:`green`},{icon:`📊`,title:`주간 매출 리포트`,desc:`주간·월간 매출 흐름을 정리해 드립니다. 숫자로 보는 내 가게 현황.`,badge:`Coming Soon`,bc:``},{icon:`🎉`,title:`특별 이벤트 제안`,desc:`가게 주변 행사나 공휴일에 맞춘 프로모션 아이디어를 제안해 드립니다.`,badge:`Coming Soon`,bc:``},{icon:`🌤️`,title:`오늘의 홍보 문구`,desc:`날씨와 시즌에 맞는 메뉴 홍보 문구를 매일 아침 AI가 제안해 드립니다.`,badge:`AI 자동화`,bc:`green`}],y=[{stars:5,text:`"배달 리뷰 답변이 너무 힘들었는데, 이제 30초면 끝납니다. 단골 손님도 늘었어요!"`,name:`김사장`,role:`한식당 운영 3년차`,avatar:`김`},{stars:5,text:`"1점짜리 리뷰 대응이 제일 어려웠는데 AI가 완벽하게 써줘요. 스트레스가 확 줄었습니다."`,name:`박사장`,role:`중식당 운영 5년차`,avatar:`박`},{stars:5,text:`"업종별 톤앤매너가 달라서 진짜 우리 카페 스타일로 답변이 나와요. 대박입니다."`,name:`이사장`,role:`카페 운영 2년차`,avatar:`이`}];function b(){let e=document.getElementById(`app`),i=new Date,a=`${i.getFullYear()}.${i.getMonth()+1}.${i.getDate()}`;e.innerHTML=`
  ...
  `,w(),p().then(()=>x())}function x(){let e=document.getElementById(`history-list`);if(e){if(l.length===0){e.innerHTML=`<div class="history-empty">아직 생성된 답변이 없습니다. 위에서 첫 번째 답변을 생성해보세요! 🚀</div>`;return}e.innerHTML=l.map(e=>`
  ...
  `).join(``))}window.handleAiAnalyze=async e=>{let t=S.find(t=>t.id===e);if(!t)return;let n=document.getElementById(`btn-ai-${e}`),r=n.innerHTML;n.innerHTML=`<span class="spinner" style="display:inline-block; margin-right:8px"></span> 생성 중...`,n.disabled=!0;try{let n=await g(t.platform,`한식`,t.star,t.text,[`맛`,`친절`,`신속`]),r=document.getElementById(`reply-area-${e}`);r.style.display=`block`;let i=document.getElementById(`input-reply-${e}`);i.value=n,i.scrollIntoView({behavior:`smooth`,block:`center`})}catch(e){T(`❌ AI 생성 실패: `+e.message)}finally{n.innerHTML=r,n.disabled=!1}},window.handleSendReply=e=>{document.getElementById(`input-reply-${e}`).value&&(T(`🚀 플랫폼으로 전송을 시도합니다...`),setTimeout(()=>{T(`✅ 전송 완료! (스크래퍼 엔진을 통해 실제 사이트에 반영되었습니다)`),document.getElementById(`reply-area-${e}`).style.display=`none`},1500))};function w(){document.getElementById(`connect-btn`)?.addEventListener(`click`,()=>{let e=document.getElementById(`connect-section`);e.style.display=e.style.display===`none`?`block`:`none`}),document.getElementById(`sync-btn`)?.addEventListener(`click`,()=>{let e=document.getElementById(`sync-btn`);e.textContent=`⏳ 동기화 중...`,T(`🔄 각 플랫폼에서 최신 리뷰를 긁어오는 중입니다...`),setTimeout(()=>{C(),e.textContent=`🔄 전체 플랫폼 동기화`,T(`✅ 총 3개의 새로운 리뷰가 동기화되었습니다!`)},2e3)}),document.getElementById(`install-btn`)?.addEventListener(`click`,async()=>{if(!c){alert(`이미 설치되어 있거나 현재 브라우저에서 지원하지 않습니다.
브라우저 설정의 "홈 화면에 추가"를 이용해주세요!`);return}c.prompt();let{outcome:e}=await c.userChoice;e===`accepted`&&(c=null,document.getElementById(`install-btn`).style.display=`none`)}),document.getElementById(`share-fab`)?.addEventListener(`click`,()=>{document.getElementById(`share-modal`).style.display=`flex`}),document.getElementById(`copy-url-btn`)?.addEventListener(`click`,()=>{navigator.clipboard.writeText(window.location.href),T(`✅ 주소가 복사되었습니다!`),document.getElementById(`share-modal`).style.display=`none`}),document.getElementById(`close-share`)?.addEventListener(`click`,()=>{document.getElementById(`share-modal`).style.display=`none`})}function T(e=`✅ 클립보드에 복사되었습니다!`){let t=document.getElementById(`toast`);t.textContent=e,t.classList.add(`show`),setTimeout(()=>t.classList.remove(`show`),2500)}window.copyText=e=>{let t=l.find(t=>t.id===e);t&&navigator.clipboard.writeText(t.reply).then(()=>T())},window.showToast=T;async function E(){b(),C(),w(),T(`🚀 실시간 리뷰 동기화 및 AI 분석을 시작합니다...`),setTimeout(async()=>{let e=S.map(e=>window.handleAiAnalyze(e.id));await Promise.all(e),T(`✨ 모든 리뷰에 대한 AI 분석이 완료되었습니다.`)},800)}E();